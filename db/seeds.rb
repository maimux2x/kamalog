...
rice_bowl = user.pieces.find_or_initialize_by(title: 'お茶碗').tap {
  it.update! studio: Studio.first, form_method: :electric_wheel
}
...
dish = user.pieces.find_or_initialize_by(title: 'お皿').tap {
  it.update! studio: Studio.first, form_method: :hand_building, status: :complete, description: '友達に上げるためのお皿を作った。'
}
...
cup = user.pieces.find_or_initialize_by(title: 'マグカップ').tap {
  it.update! studio: Studio.first, form_method: :coil_building
}
...
base = user.pieces.find_or_initialize_by(title: '花瓶').tap {
  it.update! studio: Studio.first, form_method: :coil_building, status: :complete, description: '一輪挿しを作った。'
}
